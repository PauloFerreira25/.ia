---
name: fastify
description: "Read before writing or modifying any Fastify backend code."
---
...
Fastify-specific files that every project must have:
...
**Route plugin** imports only the default instances — no dependency list needed:
...
**Override em testes** — injeta mocks apenas nas camadas mais baixas (service, repository), nunca no handler:
```typescript
...
**Override em multi-datasource** — injeta o banco certo:
```typescript
...
Routes live in `<domain>/<domain>.router.ts` and are registered as Fastify plugins. Each domain exports a plugin that registers its routes and receives its dependencies.
...
Two error handlers cover all cases. Register them in `infra/api.ts`.

`errorHandler` maps `AppError` subclasses to the HTTP envelope. Everything else returns `500` without exposing internal details:
...
// import infrastructure initializers from 30+ (e.g. initDatabase, initLogger)
